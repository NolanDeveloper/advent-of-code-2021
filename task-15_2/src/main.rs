...
fn rebuild(numbers: DMatrix<u32>) -> DMatrix<u32> {
    let r = numbers.nrows();
    let c = numbers.ncols();
    DMatrix::<u32>::from_fn(5 * numbers.nrows(), 5 * numbers.ncols(), |row, col| {
        let k = ((row / r) + (col / c)) as u32;
        let x = numbers[(row % r, col % c)];
        let t = x + k;
        if t < 10 { t } else { t % 10 + 1 }
    })
}

fn main() {
    let numbers = rebuild(read_input());
